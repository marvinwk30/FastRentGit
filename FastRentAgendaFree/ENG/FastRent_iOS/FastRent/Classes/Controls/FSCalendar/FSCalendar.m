...
- (void)reloadDataForCellatDate:(NSDate *)date {
    
    NSIndexPath *dateIndexPath = [self indexPathForDate:date];
    ...
    [_collectionView.indexPathsForVisibleItems enumerateObjectsUsingBlock:^(NSIndexPath *indexPath, NSUInteger idx, BOOL *stop) {
        ...
        if ([dateIndexPath compare:indexPath] == NSOrderedSame) {
            [self reloadDataForCell:cell atIndexPath:indexPath];
            return;
        }
    }];
...
- (void)scrollToBeginingOfMonthForDate:(NSDate *)date animated:(BOOL)animated {
    
    [self scrollToPageForDate:[self beginingOfMonthOfDate:date] animated:animated];
}
